		...
		<div class="cont_bar">
			...
			<div class="cont_bar_inner">
				...
				<div class="next_btn open_gift_btn">
					...
					<?php /*?><input type="submit" onclick="shop_cart()" class="orange" value="Checkout"><?php */?>
					<?php } else { ?>
					<?php /*?><input type="button"  id="shoping_cart" class="orange" value="Checkout"><?php */?>
					<?php } ?>
				</div>
				<div class="or your_sugst"><div class="line"><span>Your Suggestions</span></div></div>
				<ul class="gift_box your_suggst">
					<?php	include('tbl_ext.php');
					$proId = $get_dev['proid'];
					...
					if($proid) {
						foreach($proid as $pro )
						{
							...
							{ $url_iamge=$product['image_code'];
							  $affiliate_URL  =getdataA2 ('<a href= ',' ><IMG',$url_iamge);
					?>
					<li>
						<div class="img_box"><?php if($product['img'] != ''){ ?><img src="<?php echo  $product['img']; ?>" width="190" height="190" alt="Suggestion Image"  /><?php }else{ ?><img src="<?php  get_image($product['image_code']);?>" width="190" height="190" alt="Suggestion Image"  /><?php } ?></div>
						...
						<div class="terms">
							...
							<?php if($product['img'] != ''){ ?>
                            <a href="<?php echo $product['image_code'] ?>" target="_blank">Buy Now</a>
							<?php }else{ ?>  <a href="<?php echo $affiliate_URL; ?>" target="_blank">Buy Now</a> <?php } ?>
							<?php /*?><label>Add to Cart</label><label>Buy Now</label><?php */?>
						</div>
