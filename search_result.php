...
//include_once('../config/config.php');
 ...
 $query = "select delivery_id,cash_amount,gender,age,occassionid,recp_email from ".tbl_delivery." where delivery_id = '".$recption_id."'";
 $get_info = $db->get_row($query,ARRAY_A);
 $cash = $get_info['cash_amount'];
...
// echo $price_to;
 ...
 //echo $price_to;
  ...
  $price_to1 = $_GET['o'] + ($_GET['o'] * 10/100);*/
   $price_to2=str_replace("$","",$_GET['s']);
   $price_from2=str_replace("$","",$_GET['o']);
   $price_from = $price_to2 - ($price_to2 * 10/100);
   $price_to = $price_to2 + ($price_to2 * 10/100);
   $price_from1 = $price_from2 - ($price_from2 * 10/100);
   $price_to1 = $price_from2 + ($price_from2 * 10/100);
 } else if($_GET['s'] != '') {	
 $keyword = $_GET['s'];
 }
...
if(isset($_SESSION['recipit_id']['New']) || isset($_SESSION['DRAFT']['delivery_id'])) {
						...
						$search_query = "select * from ".tbl_product." where (status = 1 or status = 0) $price and hide_id not like '%".$_SESSION['LOGINDATA']['USERID']."%' limit 0,200";
						} else {
						 $search_query = "select * from ".tbl_product." where (sub_category like '%".mysql_real_escape_string(stripslashes(trim($keyword)))."%' or category like '%".mysql_real_escape_string(stripslashes(trim($keyword)))."%' or pro_name like '%".mysql_real_escape_string(stripslashes(trim($keyword)))."%') and (status = 1 or status = 0) and hide_id not like '%".$_SESSION['LOGINDATA']['USERID']."%' limit 0,200";
					 }
} else {
	if($_GET['s'] != ''  && $_GET['o'] != '' && $_GET['s'] != ''  && $_GET['o'] != '') {
							$search_query = "select * from ".tbl_product." where (status = 1 or status = 0) $price and hide_id not like '%".$_SESSION['LOGINDATA']['USERID']."%' limit 0,200";
...
/***********************GET NEXT PRODUCT********************************/
if(isset($_SESSION['recipit_id']['New'])  || isset($_SESSION['DRAFT']['delivery_id'])) {
 $nxtsql = "SELECT proid,love_id FROM ".tbl_product." WHERE proid>{$view_product['proid']} and (sub_category like '%".mysql_real_escape_string(stripslashes(trim($keyword)))."%' or category like '%".mysql_real_escape_string(stripslashes(trim($keyword)))."%' or pro_name like '%".mysql_real_escape_string(stripslashes(trim($keyword)))."%') and (status = 1 or status = 0) $owndata $hidedata and hide_id not like '%".$_SESSION['LOGINDATA']['USERID']."%'";
 if(@mysql_num_rows($chk_users) > 0) {
 $nxtsqls = "UNION SELECT proid,love_id FROM ".tbl_product." WHERE proid>{$view_product['proid']} and (sub_category like '%".mysql_real_escape_string(stripslashes(trim($keyword)))."%' or category like '%".mysql_real_escape_string(stripslashes(trim($keyword)))."%' or pro_name like '%".mysql_real_escape_string(stripslashes(trim($keyword)))."%') $price and (status = 1 or status = 0) $owndata $hidedata and hide_id not like '%".$_SESSION['LOGINDATA']['USERID']."%'";
 }
	$next_pro =  $nxtsql." ".$nxtsqls."ORDER BY proid LIMIT 1";
} else {
	$nxtsql = "SELECT proid,love_id FROM ".tbl_product." WHERE proid>{$view_product['proid']} and (sub_category like '%".mysql_real_escape_string(stripslashes(trim($keyword)))."%' or category like '%".mysql_real_escape_string(stripslashes(trim($keyword)))."%' or pro_name like '%".mysql_real_escape_string(stripslashes(trim($keyword)))."%') $price and (status = 1 or status = 0) and hide_id not like '%".$_SESSION['LOGINDATA']['USERID']."%'";
...
/***********************GET PREVIOUS PRODUCT********************************/	 					
					...
					<?php if($view_product) { ?>	
						<div class="prod_img">
						<?php if($view_product['img'] != ''){ ?><img src="<?php  echo $view_product['img']; ?>" height="280" alt="<?php echo  $view_product['pro_name'];?>" /><?php }else{ ?><img src="<?php  get_image($view_product['image_code']);?>" height="280" alt="<?php echo  $view_product['pro_name'];?>" /><?php } ?>
					...
					<input type="hidden" name="type" id="type"/>
					 <?php 
						 ...
						 <?php  
						 $url_iamges=$view_product['image_code'];
						 $affiliate_URLS  =getdataA2('<a href= ',' ><IMG',$url_iamges); 
						?>
						<?php if($view_product['img'] != '') { ?>
							<a href="<?php echo $view_product['image_code']; ?>"  target="_blank">Purchase</a>	
						<?php }else{?>
			...
			<div class="item_check">
				...
				<div class="show_option">
					...
					if($view_pro)
					{
						...
						{
					?>
						<li onclick="get_product('<?php echo $product['proid']; ?>')"><?php if($product['img'] != ''){ ?><img src="<?php echo $product['img']; ?>" alt="<?php echo $product['pro_name']; ?>" width="92" height="92" id="countButton_<?php echo $product['proid']; ?>"/><?php }else{ ?><img src="<?php get_image($product['image_code']); ?>" alt="<?php echo $product['pro_name']; ?>" width="92" height="92" id="countButton_<?php echo $product['proid']; ?>"/><?php } ?></li>
					<?php } } ?>	
					</ul>
				</div>
			</div>
