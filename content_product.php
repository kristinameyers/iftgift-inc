<?php
	...
	$chk_users = mysql_query("select userId,email from ".tbl_user." where email = '".$get_recp['recp_email']."'");
	if(mysql_num_rows($chk_users) > 0) {
		...
		$query_udemo = "Union select * from ".tbl_product." where (status = 1 or status = 0) $get_data $get_price $owndata $hidedata $lovedata ORDER BY FIND_IN_SET('".$uId."',love_id) DESC $limit ";
	}
	...
	$query_demo = "select * from ".tbl_product." where (status = 1 or status = 0) $get_data $cart_query $get_price $get_occassion_data $login_check   $limit ";
...
/***********************GET NEXT PRODUCT********************************/
 	...
 	if(@mysql_num_rows($chk_users) > 0) {
 		$nxtsqls = "UNION SELECT proid FROM ".tbl_product." WHERE proid>{$view_product['proid']} $get_data $get_price and (status = 1 or status = 0) $owndata $hidedata and hide_id not like '%".$_SESSION['LOGINDATA']['USERID']."%' $lovedata ORDER BY proid LIMIT 1";
 	}
...
/***********************GET PREVIOUS PRODUCT********************************/
	...
	if(@mysql_num_rows($chk_users) > 0) {
		$prevsqls = "UNION SELECT proid FROM ".tbl_product." WHERE proid<{$view_product['proid']} $get_data $get_price and (status = 1 or status = 0) $owndata $hidedata and hide_id not like '%".$_SESSION['LOGINDATA']['USERID']."%' $lovedata ORDER BY proid DESC LIMIT 1";
	}
...
?>
		<div class="product_bar">
			<div class="left_arrow" id="getPicButton_<?php echo $previd;?>">
				<?php if($previd != '') { ?>
					<img src="<?php echo ru_resource; ?>images/left_arrow.png" alt="Left Arrow" />
				<?php } ?>
			</div>
		<?php if($view_product) { ?>	
			<div class="prod_img">
			...
			<?php } ?><?php /*?><img src="<?php  get_image($view_product['image_code']);?>" height="280" alt="<?php echo  $view_product['pro_name'];?>" /><?php */?>
			</div>
		<?php } ?>	
		<?php if($nextid != '') { ?>	
