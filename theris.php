<?php 
 $query_demo = "select * from ".tbl_product." where (status = 1 or status = 0) and (love_id not like '0' and !FIND_IN_SET('".$_SESSION['LOGINDATA']['USERID']."',love_id))";

 $select_products =  $query_demo;
 $view_product = $db->get_row($query_demo,ARRAY_A);	
	 
/***********************GET NEXT PRODUCT********************************/
 $nxtsql = "SELECT proid FROM ".tbl_product." WHERE proid>{$view_product['proid']} and (status = 1 or status = 0) and (love_id not like '0' and !FIND_IN_SET('".$_SESSION['LOGINDATA']['USERID']."',love_id)) ORDER BY proid LIMIT 1";
...
$prevsql = "SELECT proid FROM ".tbl_product." WHERE proid<{$view_product['proid']} and (status = 1 or status = 0) and (love_id not like '0' and !FIND_IN_SET('".$_SESSION['LOGINDATA']['USERID']."',love_id)) ORDER BY proid LIMIT 1";
...
?>
	...
	<?php include_once("common/theris_right.php");?>
</div>		
